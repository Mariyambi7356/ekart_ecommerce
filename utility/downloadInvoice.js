  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(20)
        // .fontSize(4)
        .text("teccas ", 200, 65, { align: "right" })        
        .moveDown();
      ...
      doc
        ...
        .text(`Order Id: ${order._id}`, 50, 200)
         .text(`Order Date: ${order.start_date.toLocaleDateString()}`, 50, 215)
        .text(
          `Total Amount:Rs.${order.totalPrice}`,
          50,
          230
        )
        .text(order.address[0].name, 380, 200)
        .text(order.address[0].address +","+ order.address[0].city, 380, 213)
        .text(
          `${order.address[0].state}-${order.address[0].zip}`,
          380,
          228
        )
        .text(
      ...
      for (i = 0; i < order.item.length; i++) {
        ...
        generateTableRow(
          ...
          "",
          item[i].quantity,
          item[i].product.price,
          (item[i].product.price) * item[i].quantity
        );
       }
